<%@ page import="com.gaajula.sample.gtunes.Song" %>
...
<div class="fieldcontain ${hasErrors(bean: songInstance, field: 'album', 'error')} required">
	...
	<g:select id="album" name="album.id" from="${com.gaajula.sample.gtunes.Album.list()}" optionKey="id" required="" value="${songInstance?.album?.id}" class="many-to-one"/>
</div>
